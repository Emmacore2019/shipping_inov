...
class ShippingAccountPayment(models.Model):
    ...
    def _create_advance_payment_analytic_line(self, payment, sale_order, amount):
        ...
        if not folder.analytic_distribution:
            # Si pas de distribution, ne pas créer de ligne analytique
            return False
        ...
        analytic_line_vals = {
            'name': _('Paiement - Proforma %s') % sale_order.name,
        ...
        folder.message_post(
            body=_(
                "Paiement enregistré: %s %s pour la proforma %s (Paiement: %s)"
            ) % (
                amount,
